...
def save_render_color(image_path, color):
    ...
    def test_step(self, batch, batch_idx):
        ...
        with torch.set_grad_enabled(True):
            ...
            for i in range(number_steps):
                ...
                gt_color = rearrange(
                    batch["target"]["image"], "b v c h w -> (b v) c h w")
                for i in range(gt_color.shape[0]):
                    save_render_color(
                        f"tmp_eval/gt_{self.global_step}_{i}.png", gt_color[i])
                    save_render_color(
                        f"tmp_eval/render_{self.global_step}_{i}.png", render_color[i])
                ssim_, _, _, structure = ssim(
                    gt_color, render_color, size_average=True, data_range=1.0, retrun_seprate=True, win_size=11)
