...
class ModelWrapper(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        gaussians = self.encoder(
            batch["context"], self.global_step, visualization_dump=visualization_dump)
    ...
    def get_context_view_gaussians(self, batch):
        context = batch["context"]

        h, w = context["image"].shape[-2:]

        image1 = context["image"][:, 0:1]
        image2 = context["image"][:, 1:2]
        intrinsics1 = context["intrinsics"][:, 0:1]
        intrinsics2 = context["intrinsics"][:, 1:2]
        extrinsics1 = context["extrinsics"][:, 0:1]
        extrinsics2 = context["extrinsics"][:, 1:2]
        ...
        temp_context = {
            ...
            "intrinsics": torch.cat([intrinsics1, intrinsics2], dim=1),
            "extrinsics": torch.cat([extrinsics1, extrinsics2], dim=1),
        }
        gaussians: Gaussians = self.encoder(temp_context, self.global_step)
        view1_gaussians = Gaussians(
            means=gaussians.means[:, :h * w],
            covariances=gaussians.covariances[:, :h * w],
            harmonics=gaussians.harmonics[:, :h * w],
            opacities=gaussians.opacities[:, :h * w],
            scales=gaussians.scales[:, :h * w],
            rotations=gaussians.rotations[:, :h * w],
        )
        render1 = self.decoder.forward(
            view1_gaussians,
            extrinsics1,
            intrinsics1,
            temp_context["near"][:, 0:1],
            temp_context["far"][:, 0:1],
            (h, w),
        )
        ...
        temp_context = {
            ...
            "near": context["near"],
            "far": context["far"],
            "image": torch.cat([image2, image1], dim=1),
            "intrinsics": torch.cat([intrinsics2, intrinsics1], dim=1),
            "extrinsics": torch.cat([extrinsics1, torch.inverse(extrinsics2)], dim=1),
        }
        gaussians = self.encoder(temp_context, self.global_step)
        view2_gaussians = Gaussians(
            means=gaussians.means[:, :h * w],
            covariances=gaussians.covariances[:, :h * w],
            harmonics=gaussians.harmonics[:, :h * w],
            opacities=gaussians.opacities[:, :h * w],
        ...
        for i in range(len(scene)):
            filename = path / scene[i] / "gaussians_1.ply"
            export_ply(view1_gaussians.means[i][:h*w],
                       view1_gaussians.scales[i][:h*w],
                       view1_gaussians.rotations[i][:h*w],
                       view1_gaussians.harmonics[i][:h*w],
                       view1_gaussians.opacities[i][:h*w],
                       filename)

            filename = path / scene[i] / "render_color_1.png"
            save_image(render1.color[i][0], filename)

            filename = path / scene[i] / "gaussians_2.ply"
            export_ply(view2_gaussians.means[i][:h*w],
                       view2_gaussians.scales[i][:h*w],
                       view2_gaussians.rotations[i][:h*w],
                       view2_gaussians.harmonics[i][:h*w],
                       view2_gaussians.opacities[i][:h*w],
                       filename)

            filename = path / scene[i] / "render_color_2.png"
            save_image(render2.color[i][0], filename)
    ...
    def test_step(self, batch, batch_idx):
        ...
        self.get_context_view_gaussians(batch)
        ...
        if self.test_cfg.compute_scores:
            ...
            self.print_preview_metrics(
                all_metrics, methods, overlap_tag=overlap_tag)
    ...
    def test_step_align(self, batch, gaussians):
        ...
        with torch.set_grad_enabled(True):
            cam_rot_delta = nn.Parameter(torch.zeros(
                [b, v, 3], requires_grad=True, device=self.device))
    ...
    def on_test_end(self) -> None:
        name = get_cfg()["wandb"]["name"]
        self.benchmarker.dump(
            self.test_cfg.output_path / name / "benchmark.json")
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        if self.distiller is not None:
            with torch.no_grad():
                pseudo_gt1, pseudo_gt2 = self.distiller(
                    batch["context"], False)
            depth1, depth2 = pseudo_gt1['pts3d'][..., -
                                                 1], pseudo_gt2['pts3d'][..., -1]
            ...
            comparison = hcat(
                add_label(vcat(*dust_vis), "Context"), comparison)
